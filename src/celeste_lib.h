...
char *bump_alloc(BumpAllocator *bumpAllocator, size_t size)
{
  ...
  // check if there is room for memory allocation
  // align the used memory and compare it with total capacity
  if (bumpAllocator->used + allignedSize <= bumpAllocator->capacity)
  {
    // move pointer to point newly allocator memory
    result = bumpAllocator->memory + bumpAllocator->used;
    bumpAllocator->used += allignedSize; // memory is going to align by itself
  }
  else
  {
...
char* read_file(const char* filePath, int* fileSize, BumpAllocator* bumpAllocator)
{
  ...
  if(fileSize2)
  {
    ...
    fileContent = read_file(filePath, fileSize, buffer);
  }
...
bool copy_file(const char* fileName, const char* outputName, BumpAllocator* bumpAllocator)
{
  ...
  if(fileSize2)
  {
    ...
    return copy_file(fileName, outputName, buffer); // use custom-allocated memory
  }
